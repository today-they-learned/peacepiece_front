...
const useDeleteSuggestionFeedbackMuation = (
  suggestionId: number,
  main?: boolean
) => {
  return useMutation(
    () => {
      return challengeSuggestionAPI.feedback.delete(suggestionId);
    },
    {
      onSuccess: () => {
          ...
          page.data.results.forEach(
            (suggestion: ChallengeSuggestionType, index: number) => {
              if (suggestion.id === suggestionId) {
                ...
                oldSuggestion.is_feedbacked = false;
...
export default useDeleteSuggestionFeedbackMuation;
